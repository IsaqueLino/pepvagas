...
export class PerfilAlterarPage implements OnInit {
  ...
  async getUsuarioPorId(id: any) {
    try {
      ...
      const response = await api.get(`/find-admin/${id}`, { headers });
  ...
  async alterar() {
    try {
      ...
      const response = await api.put(`/update-admin/${this.id}`, {
  ...
  async alterarSenha() {
    try {
      ...
      const response = await api.put(`/update-password-admin/${this.id}`, {
        senhaAtual: this.senha.senhaAtual,
        novaSenha: this.senha.novaSenha,
        updateBy: this.updateBy
      }, { headers });
  ...
  async voltar() {
    ...
    this.navCtrl.navigateBack(['/perfil-adm']);
  }
