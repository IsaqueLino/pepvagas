...
export class VagasCandidatasPage implements OnInit {
  ...
  handleRefresh(event: any) {
    this.getCandidaturas().then(() => {
      event.target.complete(); // Finaliza o efeito de "pull to refresh"
    });
  }
  ...
  redirect(ref: string) {
    this.navController.navigateForward(ref)
  }

  navigationRoot(ref: string){
    this.navController.navigateRoot(ref)
  }
  ...
  verVagaDetalhes(idVaga: string) {
    ...
    this.navController.navigateForward(['/detalhes-vaga'], {
