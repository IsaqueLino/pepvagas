...
@Component({
  ...
  templateUrl: './candidato-profile.page.html',
  styleUrls: ['./candidato-profile.page.scss'],
})
export class CandidatoProfilePage implements OnInit {
  ...
  async baixarCurriculoCadastrado() {
    try {
      // Obter o arquivo do backend
      ...
      // Verificar se o blob é válido (opcional, para depuração)
      if (!blob || blob.size === 0) {
        this.exibirMensagem('Arquivo inválido ou vazio.');
        return;
      }
      ...
      link.href = fileURL;
      link.download = `curriculo_${Date.now()}.pdf`; // Nome do arquivo com timestamp
      ...
      this.exibirMensagem('Download iniciado. Verifique a pasta Downloads.');
    } catch (error) {
      this.exibirMensagem('Erro ao baixar o currículo: ');
    }
  }
  ...
  formatAreasInteresse(areasInteresse: any[]): string {
    ...
    return areasInteresse.map(area => area.nome).join(", ");
  }
  ...
  converterParaNumero(valorString: string) {
    if (valorString == 'R$') {
      return null
    }
